...
type Stage = "content" | "facts" | "knowledge" | "summary";
...
const VALID_STAGES: Stage[] = ["content", "facts", "knowledge", "summary"];
...
function parseStage(raw: string | null): Stage {
  ...
  throw new Error(
    "Query parameter 'stage' must be one of: content, facts, knowledge, summary.",
  );
}
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (stage === "content") {
      ...
      // summary stage
      query = query
        .not("knowledge_extracted_at", "is", null)
