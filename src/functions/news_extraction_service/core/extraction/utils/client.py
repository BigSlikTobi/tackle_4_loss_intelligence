...
class HttpClient:
    ...
    def __init__(
        ...
        # Headers picked to look like a real Chrome browser fetching a feed.
        # ESPN (and similar CDNs) treat cloud egress IPs more leniently when
        # the request carries the full set of Sec-Fetch-*, Accept-Encoding,
        # and Upgrade-Insecure-Requests headers a real browser would send.
        # The bare-minimum 3-header set we used previously was returning an
        # empty <channel> from us-central1 even with a Chrome User-Agent.
        self.session.headers.update(
            {
                "User-Agent": self.user_agent,
                "Accept": "application/xml,application/rss+xml,application/atom+xml,text/xml;q=0.9,text/html;q=0.8,*/*;q=0.7",
                "Accept-Language": "en-US,en;q=0.9",
                # Only encodings the requests library decodes natively; do
                # NOT advertise brotli (`br`) — without the optional brotli
                # package installed the body is delivered as raw compressed
                # bytes and feedparser fails with "Invalid XML, line 1".
