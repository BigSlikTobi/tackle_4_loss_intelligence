"""Cloud Function entry points for URL content extraction (submit/poll/worker).

Three handlers share this deployment:

- ``submit_url_content_job``   — POST /submit (returns 202)
- ``poll_url_content_job``     — POST /poll (status or terminal-consume)
- ``run_url_content_worker``   — POST /worker (internal, token-protected)

Architecture mirrors ``article_knowledge_extraction``: submit writes a queued
row, fire-and-forget POSTs to the worker URL, returns 202 immediately. The
worker extracts content from each URL and writes terminal state. Poll reads
non-terminal statuses, or atomically delete-on-read when terminal.
"""
...
from src.functions.url_content_extraction_service import SERVICE_NAME
...
def submit_handler(request: flask.Request) -> flask.Response:
    ...
    input_payload = {
        "urls": list(req.urls),
        "options": {
            "timeout_seconds": req.options.timeout_seconds,
            "force_playwright": req.options.force_playwright,
            "prefer_lightweight": req.options.prefer_lightweight,
            "max_paragraphs": req.options.max_paragraphs,
            "min_paragraph_chars": req.options.min_paragraph_chars,
        },
    }
