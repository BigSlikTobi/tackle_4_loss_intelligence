...
class FactBatchResultProcessor:
    ...
    def __init__(
        ...
        *,
        ...
        completion_tracker: Optional[KnowledgeCompletionTracker] = None,
        continue_on_error: bool = True,
    ) -> None:
        self.writer = writer or KnowledgeWriter()
        self.resolver = resolver or EntityResolver()
        self.reader = reader or NewsFactReader()
        self.completion_tracker = completion_tracker or KnowledgeCompletionTracker()
    ...
    def process(
        ...
        *,
        ...
        if processed_fact_ids and not dry_run:
            urls_updated = self.completion_tracker.mark_complete_for_fact_ids(
                list(processed_fact_ids)
            )
            result.urls_updated = urls_updated
        elif processed_fact_ids and dry_run:
            logger.info(
                "[DRY RUN] Would evaluate knowledge completion for %d processed facts",
